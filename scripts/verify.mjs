...
check(readText('.github/workflows/deploy-pages.yml').includes('30 22 * * 1-5'), 'workflow schedules SOX primary 07:30 KST slot');
check(readText('.github/workflows/deploy-pages.yml').includes('scripts/check_sox_freshness.py'), 'workflow uses freshness gate before retries');
check(readText('.github/workflows/deploy-pages.yml').includes("if: steps.freshness.outputs.should_collect == 'true'"), 'workflow refreshes generated data only when freshness gate allows it');
check(readText('scripts/check_sox_freshness.py').includes('push_uses_committed_generated_json'), 'push events deploy committed generated JSON without regenerating uncommitted data');
