...
const pipeline = readText('scripts/refresh_sox_pipeline.py');
const publicVerifier = readText('scripts/verify_publication.py');
...
check(!workflow.includes('--allow-degraded') && !workflow.includes('--offline-ok'), 'production workflow does not publish degraded or offline fallback data');
check(pipeline.includes('--require-current') && pipeline.includes('--fail-on-degraded'), 'every collection requires current healthy data');
check(workflow.includes('scripts/refresh_sox_pipeline.py') && workflow.includes('--require-fresh'), 'workflow stages and verifies data before publication');
check(!workflow.includes('continue-on-error:'), 'automation does not hide collection or delivery failures');
...
check(workflow.includes('scripts/verify_publication.py') && publicVerifier.includes('hashlib.sha256'), 'public verifier compares every release file by SHA256');
...
check(readme.includes('--require-current') && readme.includes('refresh_sox_pipeline.py'), 'README documents staged strict collection');
